...
const INACTIVE = 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600';
...
export default function SearchFilterBar({
  ...
  const hasActiveFilter = searchTerm !== '' || statusFilter !== '' || formatFilter !== '';
  ...
  return (
    <div className="flex flex-col gap-3 mb-5">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {hasActiveFilter && (
          <button
            onClick={() => { onSearchChange(''); onStatusChange(''); onFormatChange(''); }}
      ...
      <div className="flex flex-wrap gap-4">
            ...
            {STATUS_OPTIONS.map(({ value, label }) => (
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium rounded-lg border transition-colors ${
                  statusFilter === value ? STATUS_STYLES[value] : INACTIVE
                }`}
              >
                {label}
              </button>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">Format</span>
          <div className="flex gap-1.5">
            {FORMAT_OPTIONS.map(({ value, label }) => (
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium rounded-lg border transition-colors ${
                  formatFilter === value ? FORMAT_STYLES[value] : INACTIVE
                }`}
              >
                {label}
              </button>
